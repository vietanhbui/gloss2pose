...
def get_pose_files(lookup_folder, pose_ids):
    ...
    for pose_id in pose_ids:
        pose_filename = "{}.mov".format(pose_id)
...
def video_to_jpg(video_filepath, jpg_directory):
    unformatted_cmd = "ffmpeg -i {} -vf scale=512:512 {} -hide_banner"

    cmd = unformatted_cmd.format(
        video_filepath,
        os.path.join(jpg_directory, "%05d.jpg")
    )
...
def video_to_png(video_filepath, png_directory):
  unformatted_cmd = "ffmpeg -i {} -vf scale=512:512 {}  -hide_banner"

  cmd = unformatted_cmd.format(
    video_filepath,
    os.path.join(png_directory, "%05d.png")
  )
...
if __name__ == "__main__":
  ...
  png_directory = os.path.join(os.path.dirname(combined_video_filepath), "png")
  os.makedirs(png_directory, exist_ok=True)
  video_to_png(combined_video_filepath, png_directory)